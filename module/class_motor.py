...
class Motor:
    def __init__(
        self, pwm=100, rightIN1=36, rightIN2=28, leftIN1=37, leftIN2=40, geomag=None
    ):
    ...
    def currentblock(self, duty_R, duty_L):
        ...
        if duty_R != 0 and self.duty_R_now == 0:
            ...
            duty_R = self.duty_R_now
        if duty_L != 0 and self.duty_L_now == 0:
            duty_L = 5
        else:
            duty_L = self.duty_L_now
    ...
    def stack(self, duty_R=30, duty_L=30):
        ...
        while True:
            ...
            self.forward(
                duty_R=random.randint(int(duty_R / 2), duty_R),
                duty_L=random.randint(int(duty_L / 2), duty_L),
                time_sleep=0.05,
                tick_dutymax=5,
            )
            time.sleep(2)
            ...
            if self.angle_difference(theta_past, theta_now) < 10:
                print("stack")
            else:
                break
        self.geomag.get_mag()
