...
class GpsPhase:
  def __init__(self, motor = None, gps = None, subth=None):
    ...
    if(gps == None):
      self.Gps = class_gps.Gps()  #class_GPSのインスタンスを生成（＆GPS取得し続けるサブスレッドを起動）
    else:
      self.Gps = gps
    
    self.latitude_goal = self.Gps.goal_lati  #class_gpsでcsvファイルから取得したゴール地点のGPS情報
    self.longitude_goal = self.Gps.goal_longi

    if subth == None:
            self.subth = subthread.Subthread(gps=self.Gps, motor=self.Motor)  #subthreadのインスタンスを作成
            self.subth.run()
  ...
  def duty(self,theta_target):  #修正角度を計算
      ...
      duty_diff = Kp*theta_modify + Kd*theta_modify_diff + Ki*self.theta_modify_sum
  ...
  def detect_stack(self):  #スタック判定
      ...
      while True:
          ...
          if(math.sqrt((self.x - self.last_x)**2 + (self.y - self.last_y)**2) <= self.stack_distance):  #動けてなかったらカウント
              self.stack_state += 1
          else:
              self.stack_state = 0
  ...
  def run_stack(self):
      ...
      while(math.sqrt((self.x - before_x)**2 + (self.y - before_y)**2) <= self.stack_distance):
          ...
          if(self.turn_direction == "right"):
              ...
              self.turn_direction = "right"
          self.update_status()
          print(f"position ({before_x:5.2f}, {before_y:5.2f}) → ({self.x:5.2f}, {self.y:5.2f}) : distance = {(math.sqrt((self.x - before_x)**2 + (self.y - before_y)**2)):5.2f}")
