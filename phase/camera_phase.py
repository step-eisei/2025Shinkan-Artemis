...
class CameraPhase:
    ...
    def run(self):
        ...
        while True:
            ...
            if c1 != [-1, -1] and c2 != [-1, -1]:
                ...
                _, prop_cone = get_object_theta_and_proportion(self.cone_colour, img=roi)
                print(f"c1 = {c1}, c2 = {c2}")
            ...
            if abs(self.calc_angle(c1, c2)) <= self.angle_thres:  # red cone in the center of image
                ...
                if c1 != [-1, -1] and c2 != [-1, -1]: # red cone is in the image
                    ...
                    if dist < 100 and prop > 20: # コーンがカメラで見つからなくても、距離センサが反応して、画像が十分に赤ければ前進
                        i = 0
                        print("cone is close")
    ...
    def track_cone(self, frame, c1, c2):
        ...
        if video_flag:
            ...
            video = cv2.VideoWriter('/home/pi/TANE2025/image/video.mp4', fourcc, fps, (w, h)) 

        BUFFER = 3
        self.camera.set(cv2.CAP_PROP_BUFFERSIZE, BUFFER)

        cone_size = c2[0] - c1[0]
        dist_from_camera = math.tan(math.pi/3) / 2 * 0.1 * self.image_size[0] / cone_size
        print(f"distance estimated from yolo{dist_from_camera}")
        ...
        while((time_now - time_start) < time_run):
            ...
            for i in range(BUFFER-1):
                ...
                if video_flag: video.write(img2)

            # move towards cone
            angle = self.calc_angle([x, y], [x+w, y+h])
            if angle_past == None: angle_past = angle
